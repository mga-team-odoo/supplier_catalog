...
class product_supplier_catalog(orm.Model):
    ...
    _columns = {
        'name': fields.char('Product name', size=128, help='Name of the product'),
        'code': fields.char('Product code', size=64, help='Code of the product'),
        ...
        'weight_net': fields.float('Net Weight', digits_compute=dp.get_precision('Stock Weight'),
                               help='Net weight'),
        'date_eol': fields.date('End of life', help='Date when the product become at end of life'),
        'line_ids': fields.one2many('product.supplier.catalog.line', 'supp_cat_id', 'Lines',
                                    help='Supplier line'),
        'category_ids': fields.one2many('product.supplier.category', 'supp_cat_id', 'Categories',
                                        help='Categories for this product on the suppliers'),
        'product_id': fields.many2one('product.product', 'Product', help='Product link'),
    ...
    def _find_product(self, cr, uid, psc, context=None):
        ...
        if psc.ean13:
            p_ids = pro_obj.search(cr, uid, [('ean13', '=', psc.ean13)], context=context)
            if p_ids:
                return p_ids[0]
...
class product_supplier_catalog_line(orm.Model):
    ...
    _columns = {
        'supp_cat_id': fields.many2one('product.supplier.catalog', 'product catalog', help='Help note'),
        'supplier_id': fields.many2one('res.partner', 'Supplier', required=True, help='Supplier'),
        'name': fields.char('Product supplier name', size=128, help='Product supplier name'),
        'code': fields.char('Product supplier code', size=64, help='Product supplier code'),
        ...
        'delay': fields.integer('Delivery delay', help='Number of days to deliver the product'),
    }
...
class product_supplier_category(orm.Model):
    ...
    _columns = {
        'supp_cat_id': fields.many2one('product.supplier.catalog', 'product catalog', help='Help note'),
        'name': fields.char('Category name', size=128, help='name of teh category on the supplier'),
        'code': fields.char('Category code', size=64, help='Code of the category on the supplier'),
    }
...
class product_category(orm.Model):
    ...
    _columns = {
        'compute_coeff': fields.float(
            'Compute Coeff',
            help='Compute standard price when convert product catalog on product'),
    }
